...
{
#ifdef ELUNA
 //TODO   sEluna->OnTextEmote(player, textEmote, emoteNum, guid);
#endif
...
{
#ifdef ELUNA
 //TODO   sEluna->OnRemoveMember(guild, player, isDisbanding);
#endif
...
void ScriptMgr::OnGroupAddMember(Group* group, ObjectGuid guid)
{
    ASSERT(group);
#ifdef ELUNA
 //todo   sEluna->OnAddMember(group, guid.GetCounter());
#endif
...
void ScriptMgr::OnGroupInviteMember(Group* group, ObjectGuid guid)
{
    ASSERT(group);
#ifdef ELUNA
 //todo   sEluna->OnInviteMember(group, guid);
#endif
...
void ScriptMgr::OnGroupRemoveMember(Group* group, ObjectGuid guid, RemoveMethod method, ObjectGuid kicker, const char* reason)
{
    ASSERT(group);
#ifdef ELUNA
 //todo   sEluna->OnRemoveMember(group, guid, method);
#endif
